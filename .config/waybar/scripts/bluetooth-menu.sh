...
while true; do
  ...
  *)
    ...
    if [[ -n "$device_name" ]]; then
      ...
      # Debugging: Print device MAC for troubleshooting
      echo "Attempting to connect to: $device_mac"
      
      if [[ -n "$device_mac" ]]; then
        # Trust the device
        if bluetoothctl trust "$device_mac"; then
          notify-send "Device $device_name trusted."
        else
          notify-send "Failed to trust $device_name."
          continue
        fi
        
        # Pair the device
        if bluetoothctl pair "$device_mac"; then
          notify-send "Device $device_name paired."
        else
          notify-send "Failed to pair $device_name."
          continue
        fi
        
        # Attempt to connect
        bluetoothctl connect "$device_mac" &
        sleep 5  # Allow time for the connection attempt to complete
        
        # Check connection status
        connection_status=$(bluetoothctl info "$device_mac" | grep "Connected:" | awk '{print $2}')
        echo "Connection Status: $connection_status"  # Debugging
        ...
        notify-send "Device MAC address not found for \"$device_name\"."
      fi
    fi
    ;;
  esac
