...
while true; do
  wifi_list() {
    nmcli --fields "SECURITY,SSID" device wifi list |
      ...
      sed 's/  */ /g' |                 # Compress spaces
      sed -E "s/WPA*.?\S/󰤪 /g" |         # Replace WPA with lock icon
      sed "s/^--/󰤨 /g" |                # Replace '--' with Wi-Fi icon
      sed "s/󰤪  󰤪/󰤪/g" |                # Remove duplicate icons
      sed "/--/d" |                     # Remove '--' lines
      awk '!seen[$0]++'                 # Remove duplicate SSIDs
  }
  ...
  # Extract selected SSID, removing the icons
  read -r selected_ssid <<<"$(echo "$selected_option" | sed 's/^󰤪 //; s/^󰤨 //')"
    ...
    *)
      ...
      if echo "$saved_connections" | grep -qw "$selected_ssid"; then
        ...
        if [[ "$selected_option" =~ "󰤪" ]]; then
          ...
          if nmcli device wifi connect "$selected_ssid" | grep -q "successfully"; then
            notify-send "Connected to \"$selected_ssid\"."
          else
            notify-send "Failed to connect to \"$selected_ssid\"."
          fi
